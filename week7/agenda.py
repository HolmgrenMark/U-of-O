...
class Appt:
    ...
    def __init__(self, day, begin, end, desc):
        ...
        Raises: 
        	ValueError if appointment is not a positive span of time
                (if it ends immediately when it begins, or before)
        ...
        if self.end < self.begin:
            raise ValueError("The appointment ends before it begins")
        self.desc = desc
        return
    ...
    def __lt__(self, other):
        ...
        h_other = other.begin.hour
        m_other = other.begin.minute
        eh_other = other.end.hour
        em_other = other.end.minute
    ...
    def overlaps(self, other):
		...
		Arguments two appointments.  
        Returns True if they overlap.

		(Hint:  Implement the __lt__ and __gt__ methods first, 
		 so you can use < and > here.)    
    ...
    def intersect(self, other, desc=""):
        ...
        if self.begin < other.begin:
            later = other.begin.time()
        else:
            later = self.begin.time()

        if self.end < other.end:
            earlier = self.end.time()
        else:
            earlier = other.end.time()

        return Appt(self.begin.date(), later,
                     earlier, desc)
...
class Agenda:
    ...
    def intersect(self,other,desc=""): 
        ...
        for old in other:
            for new in self:
                if new.overlaps(old):
                    result.append(new.intersect(old))
